...
<div class="activity_feed_header">
	<div class="activity_feed_header_info">
		{$aFeed|user:'':'':50}{if (!empty($aFeed.parent_module_id) || isset($aFeed.parent_is_app))} {_p var='shared'}{else}{if isset($aFeed.parent_user)} {img theme='layout/arrow.png' class='v_middle'} {$aFeed.parent_user|user:'parent_':'':50} {/if}{if !empty($aFeed.feed_info)}<span class="feed_info"> {$aFeed.feed_info}</span>{/if}{/if}
		<time>
