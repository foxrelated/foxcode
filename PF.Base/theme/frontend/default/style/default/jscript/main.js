...
$Behavior.globalThemeInit = function()
{
	...
	*/
	...
	$('#js_compose_new_message:not(.instant_message) #message').keydown(function(){$Core.resizeTextarea($(this));});
      ...
      drag: function(event, ui) {
        ...
        ui.position.left = newLeft;
			...
			stop: function(evt,ui){
				$.ajaxCall(sModule +'.repositionCoverPhoto', 'id='+iId + '&position=' + ui.position.top);
			}
		}).parent().parent().append('<div id="save_reposition_cover" class="btn btn-primary" onclick="$(\'.pages_header_cover img, .profiles_banner_bg .cover img\').draggable(\'destroy\');$(\'.profiles_banner\').removeClass(\'editing\');$(\'#save_reposition_cover\').remove();">' + oTranslations['save'] + '</div>');
	}
