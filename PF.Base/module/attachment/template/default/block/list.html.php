	...
	{foreach from=$aAttachments key=iKey item=aAttachment}
		<div class="attachment_row" id="js_attachment_id_{$aAttachment.attachment_id}">				
			...
			<div class="attachment_row_title">
				...
				<a href="#" class="attachment_row_link no_ajax_link" onclick="$.ajaxCall('attachment.playVideo', 'attachment_id={$aAttachment.attachment_id}', 'GET'); return false;">{$aAttachment.file_name}</a> ({if !empty($aAttachment.video_duration)}{$aAttachment.video_duration}, {/if}{_p var='views' total=$aAttachment.counter})
				{elseif !$bIsAttachmentNoHeader}			
				<a href="{url link='attachment.download' id=''$aAttachment.attachment_id''}" class="attachment_row_link no_ajax_link">{$aAttachment.file_name}</a> ({$aAttachment.file_size|filesize}, {_p var='views' total=$aAttachment.counter})
				{else}
				...
				{if (Phpfox::getUserParam('attachment.delete_own_attachment') && $aAttachment.user_id == Phpfox::getUserId()) || Phpfox::getUserParam('attachment.delete_user_attachment')}
					- <a href="#" onclick="$Core.jsConfirm({left_curly}{right_curly},function(){left_curly}  $.ajaxCall('attachment.delete', 'id={$aAttachment.attachment_id}');{right_curly},function(){left_curly}{right_curly}); return false;">{_p var='delete'}</a>
				{/if}
