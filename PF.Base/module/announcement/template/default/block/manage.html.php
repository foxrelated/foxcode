...
<table cellpadding="0" cellspacing="0">
	...
	<tr class="{if is_int($iKey/2)} tr{else}{/if}">
		<td class="t_center">
			...
			<div class="link_menu">
				<ul>
					<li><a href="{url link='admincp.announcement.add' id=$aAnnouncement.announcement_id}">{_p var='edit'}</a></li>
					<li><a href="{url link='admincp.announcement' delete=$aAnnouncement.announcement_id}" data-message="{_p var='are_you_sure' phpfox_squote=true}" class="sJsConfirm">{_p var='delete'}</a></li>
				</ul>
			</div>		
		</td>
		...
		<td class="t_center">
			<div class="js_item_is_active"{if !$aAnnouncement.is_active} style="display:none;"{/if}>
				<a href="#?call=announcement.setActive&amp;id={$aAnnouncement.announcement_id}&amp;active=0" class="js_item_active_link" title="{_p var='deactivate'}">{img theme='misc/bullet_green.png' alt=''}</a>
			</div>
			<div class="js_item_is_not_active"{if $aAnnouncement.is_active} style="display:none;"{/if}>
				<a href="#?call=announcement.setActive&amp;id={$aAnnouncement.announcement_id}&amp;active=1" class="js_item_active_link" title="{_p var='activate'}">{img theme='misc/bullet_red.png' alt=''}</a>
			</div>
		</td>	
	</tr>
	{/foreach}
