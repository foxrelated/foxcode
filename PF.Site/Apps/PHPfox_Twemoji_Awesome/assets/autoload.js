...
(function($){
	$.fn.emoji = function(){
		...
		return this.each(function(){
			var regex = new RegExp(':(' + emojiKeys + '):', 'g');
	...
	$.fn.emoji.replace = function(){
		...
		key = key.replace(/[#_]/g,'-');
		return '<i class="twa twa-' + key + '"></i>';
	};
})(jQuery);

var addEmoji = function(key) {
	emojiInsertAtCaret(lastEmojiObject, key);
};

var emojiInsertAtCaret = function(areaId, text) {
	Editor.setId(lastEmojiObject.attr('id'));
	Editor.insert({
		"type" : "emoticon",
		"text" : text
	});
...
$Ready(function() {
	$(twemoji_selectors).not('.twa_built').each(function(i, d) {
	...
	if ($('.global_attachment_list').length) {
		var total = 0;
		$('.global_attachment_list:not(._twa_built)').each(function() {
			var t = $(this),
				id =  t.data('id');

			total++;
			t.addClass('_twa_built');
			t.append('<li><a href="' + PF.url.make('/emojis',{id: id}) + '" class="emoji-popup no_ajax js_hover_title"><i class="fa fa-smile-o"></i><span class="js_hover_info">Emoji</span></a></li>');
		});
	...
	$('.emoji-popup').unbind().click(function() {
		var editorId  = $(this).closest('.global_attachment_list').data('id');
		lastEmojiObject = $(this).closest('form').find('textarea#'+editorId),
			t = $(this);
		var js_box = $('.js_box_content'), is_popup = false;
		if (js_box.length) {
	...
	var cheatSheet = $('#emoji-cheat-sheet');
	if (cheatSheet.length) {
		var html = '', keys = emojiKeys.split('|');

		html += '<ul class="emoji-list">';
		for (var i in keys) {
			var e = keys[i], name = e.replace(new RegExp('_', 'g'), '-');

			html += '<li><i class="twa twa-' + name + '"></i><span>:' + e + ':</span></li>';
		}
		html += '</ul>';
