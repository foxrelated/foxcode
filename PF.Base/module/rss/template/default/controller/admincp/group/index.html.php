...
<table id="js_drag_drop" cellpadding="0" cellspacing="0">
	...
	<tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
		...
		<td class="t_center">
			...
			<div class="link_menu">
				<ul>
					<li><a href="{url link='admincp.rss.group.add' id={$aGroup.group_id}">{_p var='edit_group'}</a></li>
		...
		<td>{_p var=$aGroup.name_var}</td>
		<td class="t_center">
			<div class="js_item_is_active"{if !$aGroup.is_active} style="display:none;"{/if}>
				<a href="#?call=rss.updateGroupActivity&amp;id={$aGroup.group_id}&amp;active=0" class="js_item_active_link" title="{_p var='deactivate'}">{img theme='misc/bullet_green.png' alt=''}</a>
			</div>
			<div class="js_item_is_not_active"{if $aGroup.is_active} style="display:none;"{/if}>
				<a href="#?call=rss.updateGroupActivity&amp;id={$aGroup.group_id}&amp;active=1" class="js_item_active_link" title="{_p var='activate'}">{img theme='misc/bullet_red.png' alt=''}</a>
			</div>		
		</td>		
	</tr>
	{/foreach}
