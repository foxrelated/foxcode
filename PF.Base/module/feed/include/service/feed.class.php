...
class Feed_Service_Feed extends Phpfox_Service
{
	...
	public function get($iUserId = null, $iFeedId = null, $iPage = 0, $bForceReturn = false, $bLimit = true, $iLastFeedId = null)
	{
		...
		if (isset($iUserId) && ($iUserId == Phpfox::getUserId())) {
			$extra .= ' AND (feed.parent_user_id=0 OR feed.parent_user_id = ' . (int)Phpfox::getUserId() . ')';
		}
		...
		if ((Phpfox::getParam('feed.allow_choose_sort_on_feeds') && Phpfox::getUserBy('feed_sort')) || defined('PHPFOX_IS_USER_PROFILE')) {
			...
			if (!empty($aLastFeed) && !empty($aLastFeed['time_stamp'])) {
				$sLoadMoreCond = 'AND feed.time_stamp < ' . (int)$aLastFeed['time_stamp'];
			}
		...
		if (isset($this->_aCallback['module'])) {
            ...
            if (Phpfox::isUser()) {
                $aBlockedUserIds = User_Service_Block_Block::instance()->get(null, true);
                if (!empty($aBlockedUserIds)) {
                    $aCustomCondition[] = $aNewCond[] = 'AND feed.user_id NOT IN (' . implode(',', $aBlockedUserIds) . ')';
                }
            }
		...
		elseif ($iUserId !== null) {
			...
			if ($iUserId == Phpfox::getUserId()) {
                ...
                $oUserObject = User_Service_User::instance()->getUserObject($iUserId);
	...
	public function getPhraseForLikes(&$aFeed, $bForce = false)
	{
		...
		if (Phpfox::isModule('like')) {
			$oLike = Like_Service_Like::instance();
		}
		...
		if (Phpfox::getParam('feed.cache_each_feed_entry')) {
			...
			if (!isset($aFeed['feed_is_liked'])) {
				if (Phpfox::isModule('like')) {
					$aFeed['feed_is_liked'] = Like_Service_Like::instance()->didILike($aFeed['type_id'], $aFeed['item_id'], [],(isset($aFeed['feed_table_prefix']) ? $aFeed['feed_table_prefix'] : ''));
				}
			}
		}
		...
		if (isset($aFeed['likes']) && is_array($aFeed['likes']) && $iCountLikes > 0) {
			...
			foreach ($aFeed['likes'] as $aLike) {
				if ($iIteration >= $iCountLikes) {
                    ...
                    if (Phpfox::isUser() && User_Service_Block_Block::instance()->isBlocked(null, $aLike['user_id']))
                    {
		...
		if (isset($aFeed['feed_total_like']) && $aFeed['feed_total_like'] > Phpfox::getParam('feed.total_likes_to_display') && Phpfox::getParam('feed.total_likes_to_display') != 0) {
			$sLink = '<a href="#" onclick="return $Core.box(\'like.browse\', 400, \'type_id=' . $aFeed['like_type_id'] . '&amp;item_id=' . $aFeed['item_id'] . '\');">';
			...
			$sPhrase .= '</a>' . _p('likes_this');
		} else {
			if (isset($aFeed['likes']) && count($aFeed['likes']) > 1) {
				$sPhrase .= '&nbsp;' . _p('like_this');
			} else {
				if (isset($aFeed['feed_is_liked']) && $aFeed['feed_is_liked']) {
	...
	public function getFeed($iId, $sPrefix = '')
	{
		return $this->database()->select('*')
			->from(Phpfox::getT($sPrefix . (isset($this->_aCallback['table_prefix']) ? $this->_aCallback['table_prefix'] : '') . 'feed'))
			->where('feed_id =' . (int)$iId)
			->executeRow();
	}
	...
	public function processAjax($iId)
	{
		...
		if (isset($aFeeds[0]['type_id'])) {
			Phpfox_Template::instance()->assign([
				'aFeed'         => $aFeeds[0],
				'aFeedCallback' => ['module' => str_replace('_comment', '', $aFeeds[0]['type_id']), 'item_id' => $aFeeds[0]['item_id']],
    ...
    public function processUpdateAjax($iId)
	{
        ...
        $oAjax->html('#js_item_feed_' . $iId, $sNewContent);
        $oAjax->call("tb_remove();");
        $oAjax->call('setTimeout(function(){$Core.resetActivityFeedForm();$Core.loadInit();}, 500);');
	}
	...
	private function _processFeed($aRow, $sKey, $iUserId, $bFirstCheckOnComments)
	{
		...
		if (Phpfox::isModule('like') && (isset($aFeed['like_type_id']) || isset($aRow['item_id'])) && ((isset($aFeed['enable_like']) && $aFeed['enable_like'])) || (!isset($aFeed['enable_like'])) && (isset($aFeed['feed_total_like']) && (int)$aFeed['feed_total_like'] > 0)) {
			$aFeed['likes'] = Like_Service_Like::instance()->getLikesForFeed($aFeed['like_type_id'], (isset($aFeed['like_item_id']) ? $aFeed['like_item_id'] : $aRow['item_id']), ((int)$aFeed['feed_is_liked'] > 0 ? true : false), Phpfox::getParam('feed.total_likes_to_display'), true, (isset($aFeed['feed_table_prefix']) ? $aFeed['feed_table_prefix'] : ''));
			$aFeed['feed_total_like'] = Like_Service_Like::instance()->getTotalLikeCount();
		}

		if (isset($aFeed['comment_type_id']) && (int)$aFeed['total_comment'] > 0 && Phpfox::isModule('comment')) {
			$aFeed['comments'] = Comment_Service_Comment::instance()->getCommentsForFeed($aFeed['comment_type_id'], $aRow['item_id'], Phpfox::getParam('comment.comment_page_limit'), null, null, (isset($aFeed['feed_table_prefix']) ? $aFeed['feed_table_prefix'] : ''));
		}
     ...
     * @return bool|mixed
     */
    public function getUserStatusFeed($aCallback, $iFeedId){
        //Make hash for cache
        $hash = 'hash_';
        ...
        if (!$aStatusFeed = $this->cache()->get($sCacheId)){
    ...
    public function canSponsoredInFeed($sType, $iId)
    {
        ...
        if ($aRow['is_active'] == 0) return false;
        return $aRow['item_id'];
    }
}