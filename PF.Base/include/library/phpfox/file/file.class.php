...
class Phpfox_File
{
	...
	public function load($sFormItem, $aSupported = array(), $iMaxSize = null)
	{
		...
		if (Phpfox::isUser())
		{
			if (!User_Service_Space::instance()->isAllowedToUpload(Phpfox::getUserId(), filesize($this->_aFile['tmp_name'])))
			{
				return false;
			}
		}
		...
		{
			return Phpfox_Error::set(_p('not_a_valid_file_extension_we_only_accept_support', array('support' => implode(', ', $aSupported))));
		}
		...
		{
			return Phpfox_Error::set(_p('not_a_valid_image_we_only_accept_the_following_file_extensions_support', array('support' => implode(', ', $aSupported))));
		}
	...
	if ($sPlugin = Phpfox_Plugin::get('library_phpfox_file_file_upload_2')){eval($sPlugin);if (isset($mReturnFromPlugin)){return $mReturnFromPlugin;}}
	    ...
	    else if (defined('PHPFOX_APP_USER_ID') || defined('PHPFOX_HTML5_PHOTO_UPLOAD') || Phpfox_Request::instance()->method() == 'PUT')
		{
	...
	public function forceDownload($sFile, $sName, $sMimeType = '', $sFileSize = '', $iServerId = 0)
	{
		...
		$sFile = Phpfox::getLib('cdn')->getUrl(str_replace(PHPFOX_DIR, Phpfox::getParam('core.path_file'), $sFile), $iServerId);
    ...
    private function _passLimit()
    {
		...
		*/
    	...
    	{
    		case 1:
    				return Phpfox_Error::set(_p('upload_failed_server_cannot_handle_files_larger_then_file_size', array('file_size' => ini_get('upload_max_filesize'))));
    			break;
    		default:
			    	...
			    	{
			    		return Phpfox_Error::set(_p('upload_failed_server_cannot_handle_files_size_larger_then_file_size', array('size' => $this->filesize($iSize, 0), 'file_size' => $this->filesize($iUploadMaxFileSize))));
			    	}

			    	if ($this->_iMaxSize !== null && $iSize >= ($this->_iMaxSize * 1048576))
			    	{
			    		return Phpfox_Error::set(_p('upload_failed_your_file_size_is_larger_then_our_limit_file_size', array('size' => $this->filesize($iSize, 0), 'file_size' => $this->filesize(($this->_iMaxSize * 1048576)))));
			    	}
