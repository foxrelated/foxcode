        ...
        <div class="comment_mini_content_holder_icon"{if isset($aFeed.marks) || (isset($aFeed.likes) && is_array($aFeed.likes)) || (isset($aFeed.total_comment) && $aFeed.total_comment > 0)}{else}{/if}></div>
			<div class="comment_mini_content_border">
				...
				<div class="comment_mini_content_commands">
					{if isset($aFeed.like_type_id) && !(isset($aFeed.disable_like_function) && $aFeed.disable_like_function) && !User_Service_Block_Block::instance()->isBlocked(null, $aFeed.user_id)}
					...
					{/if}
					{if Phpfox::isModule('share') && !isset($aFeed.no_share) && !empty($aFeed.type_id) && (!isset($bGroupIsShareable) || $bGroupIsShareable)  && !User_Service_Block_Block::instance()->isBlocked(null, $aFeed.user_id)}
			...
			<div class="{if isset($sFeedType) &&  $sFeedType == 'view'} feed_item_view{/if} comment_mini comment_mini_end">
				<form method="post" action="#" class="js_comment_feed_form">
					...
					<div class="{if isset($sFeedType) &&  $sFeedType == 'view'}comment_mini_content {/if}comment_mini_textarea_holder">
						...
						<textarea cols="60" rows="1" name="val[text]" class="js_comment_feed_textarea" id="js_feed_comment_form_textarea_{$aFeed.feed_id}" placeholder="{_p var='write_a_comment'}"></textarea>
