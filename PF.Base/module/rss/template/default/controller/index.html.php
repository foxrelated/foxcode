...
<div class="row3">
	<div class="row_title_group">{_p var=$sGroup}</div>
	<ul class="action">
	{foreach from=$aFeeds item=aFeed}
		{if isset($aFeed.child)}
		<li><div class="parent">{_p var=$aFeed.title_var}</div>
			<ul>
		...
		{else}
		<li><a href="{url link='rss' id=$aFeed.feed_id}" class="no_ajax_link">{img theme='rss/tiny.png' class='v_middle'} {_p var=$aFeed.title_var}</a></li>
		{/if}
	{/foreach}
	</ul>
</div>
