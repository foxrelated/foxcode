...
class User_Service_Callback extends Phpfox_Service
{
	...
	public function getFavorite($aFavorites)
	{
		...
		foreach ($aItems as $iKey => $aItem)
		{
			...
			$aItems[$iKey]['extra_info'] = _p('joined_time_stamp', array('time_stamp' => Phpfox::getTime(Phpfox::getParam('core.global_update_time'), $aItem['time_stamp'])));
		}
	...
	{
        User_Service_Block_Process::instance()->delete($iUser);
		...
		Profile_Service_Process::instance()->clearProfileCache( (int)$iUser );
        
        User_Service_Process::instance()->removeProfilePic($iUser);
	...
	public function exportModule($sProductId, $sModule = null)
	{
		...
		(User_Service_Group_Setting_Setting::instance()->export($sProductId, $sModule) ? $iCnt++ : null);
	...
	public function spamCheck()
	{
		return array(
			'phrase' => _p('users'),
			'value' => User_Service_User::instance()->getSpamTotal(),
	...
	public function getNewsFeedJoined_FeedLike($aRow, $iUserId = null)
	{
		...
		{
			$aRow['text'] = _p('a_href_user_link_full_name_a_liked_that_a_href_view_user_link_view_full_name_a_a_href_link_joined_a_the_community', array(
					'full_name' => Phpfox::getLib('parse.output')->clean($aRow['owner_full_name']),
					'user_link' => Phpfox_Url::instance()->makeUrl($aRow['owner_user_name']),
	...
	public function getNewsFeedStatus_FeedLike($aRow)
	{
		...
		{
			$aRow['text'] = _p('a_href_user_link_full_name_a_likes_a_href_view_user_link_view_full_name_a_s_a_href_link_status_a', array(
					'full_name' => Phpfox::getLib('parse.output')->clean($aRow['owner_full_name']),
					'user_link' => Phpfox_Url::instance()->makeUrl($aRow['owner_user_name']),
					'view_full_name' => Phpfox::getLib('parse.output')->clean($aRow['viewer_full_name']),
					'view_user_link' => Phpfox_Url::instance()->makeUrl($aRow['viewer_user_name']),
	...
	public function getNewsFeedPhoto_FeedLike($aRow)
	{
		...
		{
			$aRow['text'] = _p('a_href_user_link_full_name_a_likes_a_href_view_user_link_view_full_name_a_s_profile_a_href_link_photo_a', array(
					'full_name' => Phpfox::getLib('parse.output')->clean($aRow['owner_full_name']),
					'user_link' => Phpfox_Url::instance()->makeUrl($aRow['owner_user_name']),
					'view_full_name' => Phpfox::getLib('parse.output')->clean($aRow['viewer_full_name']),
					'view_user_link' => Phpfox_Url::instance()->makeUrl($aRow['viewer_user_name']),
	...
	public function getActivityFeedPhoto($aItem, $aCallback = null, $bIsChildItem = false)
	{
        ...
        if (Phpfox::isModule('like'))
        {
            ...
            $this->database()->leftJoin(Phpfox::getT('like'), 'l', 'l.type_id = \'user_photo\' AND l.item_id = p.photo_id');
            $this->database()->group('p.photo_id');

            $sSelect .= ', l2.like_id AS is_liked';
            $this->database()->leftJoin(Phpfox::getT('like'), 'l2', 'l2.type_id = \'user_photo\' AND l2.item_id = p.photo_id AND l2.user_id = ' . Phpfox::getUserId());
        }
        ...
        $aReturn = [
			'feed_title' => '',
			'feed_info' => _p('updated_gender_profile_photo', array('gender' => User_Service_User::instance()->gender($aItem['gender'], 1))),
	...
	public function addLikeStatus($iItemId, $bDoNotSendEmail = false)
	{
		...
		if (!$bDoNotSendEmail)
		{
			...
			Notification_Service_Process::instance()->add('user_status_like', $aRow['status_id'], $aRow['user_id']);
		}
	}	
	...
	public function getNotificationStatus_Like($aNotification)
	{
		...
		{
			$sPhrase = _p('user_name_liked_gender_own_status_update_title',	array('user_name' => Notification_Service_Notification::instance()->getUsers($aNotification), 				    'gender' => User_Service_User::instance()->gender($aRow['gender'], 1), 				    'title' => Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...')));
		}
		elseif ($aRow['user_id'] == Phpfox::getUserId())		
		{
			$sPhrase = _p('user_name_liked_your_status_update_title',array('user_name' => Notification_Service_Notification::instance()->getUsers($aNotification), 'title' => Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...')));
		}
		else 
		{
			$sPhrase = _p('user_name_liked_span_class_drop_data_user_full_name_s_span_status_update_title',array('user_name' => Notification_Service_Notification::instance()->getUsers($aNotification), 'full_name' => $aRow['full_name'], 'title' => Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...')));
		}
	...
	{		
        ...
        $iLastId = $this->database()->select('comment_id')
            ->from(':comment')
            ->where('item_id=' . (int) $aVals['item_id'])
            ->order('comment_id DESC')
            ->execute('getSlaveField');
        $sLink = Phpfox_Url::instance()->makeUrl($aRow['user_name'], array('status-id' => $aRow['status_id'])) . '#js_comment_text_' . $iLastId;
        Comment_Service_Process::instance()->notify(array(
				'user_id' => $aRow['user_id'],
				'item_id' => $aRow['status_id'],
				'owner_subject' => _p('full_name_commented_on_your_status_update', array('full_name' => Phpfox::getUserBy('full_name'))),
				'owner_message' => _p('full_name_commented_on_your_status_update_title_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',array('full_name' => Phpfox::getUserBy('full_name'), 'title' => Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->parse($aRow['content']), 50, '...'), 'link' => $sLink)),
				'owner_notification' => 'comment.add_new_comment',
				'notify_id' => 'comment_user_status',
				'mass_id' => 'user_status',
				'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('full_name_commented_on_one_of_gender_status_updates',array('full_name' => Phpfox::getUserBy('full_name'), 'gender' => User_Service_User::instance()->gender($aRow['gender'], 1)))
					:_p('full_name_commented_on_one_of_other_full_name_s_status_updates',array('full_name' => Phpfox::getUserBy('full_name'), 'other_full_name' => $aRow['full_name']))),
				'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? 
_p('full_name_commented_on_gender_status_update_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',array('full_name' => Phpfox::getUserBy('full_name'), 'gender' =>User_Service_User::instance()->gender($aRow['gender'], 1), 'title' => Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->parse($aRow['content']), 50, '...'), 'link' => $sLink))
					:_p('full_name_commented_on_other_full_name_s_status_update_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',array('full_name' => Phpfox::getUserBy('full_name'), 'other_full_name' => $aRow['full_name'], 'link' => $sLink, 'title' => Phpfox::getLib('parse.output')->shorten(Phpfox::getLib('parse.output')->parse($aRow['content']), 50, '...'))))
			)
		);		
	}		
	...
	public function getCommentNotificationStatus($aNotification)
	{
		...
		if ($aNotification['user_id'] == $aRow['user_id'])
		{
			$sPhrase = _p('span_class_drop_data_user_full_name_span_commented_on_gender_status_update_title',array('full_name' => $aNotification['full_name'], 'gender' => User_Service_User::instance()->gender($aRow['gender'], 1), 'title' => Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...')));
		}
		elseif ($aRow['user_id'] == Phpfox::getUserId())		
		{
			$sPhrase = _p('span_class_drop_data_user_full_name_span_commented_on_your_status_update_title',array('full_name' => $aNotification['full_name'], 'title' => Phpfox::getLib('parse.output')->shorten($aRow['content'], Phpfox::getParam('notification.total_notification_title_length'), '...')));
		}
		else 
		{
        ...
        $iLastId = $this->database()->select('comment_id')
            ->from(':comment')
            ->where('user_id=' . (int) $aNotification['owner_user_id'] .' AND item_id=' . (int) $aNotification['item_id'])
            ->order('comment_id DESC')
            ->execute('getSlaveField');
		return array(
			'link' => Phpfox_Url::instance()->makeUrl($aRow['user_name'], array('status-id' => $aRow['status_id'])) . '#js_comment_text_' . $iLastId,
			'message' => $sPhrase
		);
	}	
	...
	{
		$this->database()->select('item.user_id AS item_id, item.full_name AS item_title, item.last_login AS item_time_stamp, item.user_id AS item_user_id, \'user\' AS item_type_id, \'\' AS item_photo, 0 AS item_photo_server')
