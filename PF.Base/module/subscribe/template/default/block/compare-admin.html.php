...
<div class="message">{_p var='there_is_nothing_to_compare_at_this_time'}</div>
{/if}
...
<table id="tbl_compare" cellpadding="0" cellspacing="0">
    ...
    <tr class="trDescription">
        <td class="tdTitle">{_p var='package_info'}</td>
    ...
    <tr class="tr_feature_head">
        <td class="td_feature_head" colspan="{if ($sColspan = (count($aPackages.packages)) + 1)}{/if}{$sColspan}">{_p var='membership_package_comparison'}</td>				</tr>
    ...
    <tr id="tr_features_template" class="tr_feature" style="display:none;">
        ...
        <div>
            <span class="switch_type js_hover_title" onclick="$Core.subscribe.switchType(this);">{img theme='misc/arrow_switch.png'}<span class="js_hover_info">{_p var='toggle_comparison_value__yes__no_or_text_field_'}</span></span>
								<span class="div_text">
								...
								<span class="div_radio js_hover_title" style="display: none;">
									...
									<span class="js_hover_info">{_p var='toggle_yes_or_no'}</span>
								</span>
        </div>
        </td>
        {/foreach}
