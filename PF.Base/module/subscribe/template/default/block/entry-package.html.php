...
<div class="subscription_package {if is_int($phpfox.iteration.packages/2)}row1{else}row2{/if}{if $phpfox.iteration.packages == 1} row_first{/if}">
	<div class="item_content">
		...
		<div style="width:130px; float:left; text-align:center;">
			<a href="#" onclick="{if Phpfox::isUser()}tb_show('{_p var='select_payment_gateway' phpfox_squote=true}', $.ajaxBox('subscribe.upgrade', 'height=400&amp;width=400&amp;id={$aPackage.package_id}'));{else}$('#js_subscribe_package_id').val('{$aPackage.package_id}'); tb_remove(); {/if} return false;">
		...
		<div style="margin-left:135px; min-height:120px; height:auto !important; height:120px;">
			{/if}
			<span class="row_title_link"><a href="#" onclick="{if Phpfox::isUser()}tb_show('{_p var='select_payment_gateway' phpfox_squote=true}', $.ajaxBox('subscribe.upgrade', 'height=400&amp;width=400&amp;id={$aPackage.package_id}'));{else}$('#js_subscribe_package_id').val('{$aPackage.package_id}'); tb_remove(); {/if} return false;" class="row_title">{$aPackage.title|convert|clean}</a></span>
		...
		<div class="t_right">
			<ul class="item_menu" style="margin:0px;">
				<li><a href="#" onclick="{if Phpfox::isUser()}tb_show('{_p var='select_payment_gateway' phpfox_squote=true}', $.ajaxBox('subscribe.upgrade', 'height=400&amp;width=400&amp;id={$aPackage.package_id}'));{else}$('#js_subscribe_package_id').val('{$aPackage.package_id}'); tb_remove(); {/if} return false;">{if Phpfox::isUser() || (isset($bIsOnSignup) && $bIsOnSignup)}{_p var='select_upgrade'}{else}{_p var='upgrade'}{/if}</a></li>
			</ul>
		</div>
	</div>
</div>
