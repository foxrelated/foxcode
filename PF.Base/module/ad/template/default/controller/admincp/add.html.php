...
{/if}
	...
	<div class="table js_add_hidden" id="js_type_image" style="display:none;">
		...
		{/if}		
			<div id="js_ad_upload_banner"{if $bIsEdit} style="display:none;"{/if}>
				<input type="file" accept="image/*" name="image" size="30" />{if $bIsEdit} - <a href="#" onclick="$('#js_ad_upload_banner').hide(); $('#js_ad_banner').show(); return false;">{_p var='cancel'}</a>{/if}
				<div class="extra_info">
	...
	<div class="table_header">
		{_p var='campaign_details'}
	</div>
	<div class="table form-group">
		<div class="table_left">
		{required}{_p var='campaign_name'}:
		</div>
		<div class="table_right">
	...
	<div class="table form-group">
		...
		<div class="table_right">
			<label><input type="radio" name="val[end_option]" value="0" checked="checked" class="v_middle end_option" {value type='radio' id='end_option' default='0'}/> {_p var='do_not_end_this_campaign'}</label> <br />
			<label><input type="radio" name="val[end_option]" value="1" class="v_middle end_option" {value type='radio' id='end_option' default='1'}/> {_p var='end_on_a_specific_date'}</label>
	...
	<div class="table form-group">
		...
		<div class="table_right">
			...
			<label><input type="checkbox" name="val[view_unlimited]" id="view_unlimited" class="v_middle"{if (isset($aForms) && isset($aForms.view_unlimited)) || (!isset($aForms))} checked="checked" {/if}/> {_p var='unlimited'}</label>
		</div>
		<div class="clear"></div>
	</div>
	
	<div class="table form-group" id="js_total_click" style="display:none;">
		<div class="table_left">
			{_p var='total_clicks'}:
		</div>
		<div class="table_right">
			...
			<label><input type="checkbox" name="val[click_unlimited]" id="click_unlimited" class="v_middle"{if (isset($aForms) && isset($aForms.click_unlimited)) || (!isset($aForms))} checked="checked" {/if}/> {_p var='unlimited'}</label>
		</div>
		<div class="clear"></div>
	</div>	
	...
	{/if}
	<div class="table form-group"{if $bIsEdit && $aForms.location == 50} style="display:none;"{/if}>
		<div class="table_left">
			{_p var='placement'}:
		</div>
		<div class="table_right">
			...
			<select name="val[location]" id="location">	
				<optgroup label="Blocks">
					...
					{if $bIsEdit && Phpfox::getParam('ad.multi_ad')}
						<option value="50"{value type='select' id='location' default=50}>{_p var='block' x=50}</option>
					{/if}
				</optgroup>
	...
	<div class="table_clear">
		{if isset($aForms.is_custom) && $aForms.is_custom == '2'}
		<input type="submit" value="{_p var='approve'}" class="button btn-primary" name="val[approve]"/>
		<input type="submit" value="{_p var='deny'}" class="button btn-danger" name="val[deny]" />
		{else}
