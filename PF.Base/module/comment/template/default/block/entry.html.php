...
<div id="js_comment{$aRow.comment_id}">
	...
	<div class="comment_box p_relative">
		...
		<div class="comment_link">
			...
			{if ($aRow.view_id == 9 || $aRow.view_id == '1') && Phpfox::getUserParam('comment.can_moderate_comments')}
				<li class="js_comment_moderate"><a href="#" onclick="$('#js_comment{$aRow.comment_id}').find('.comment_header').removeClass('row_moderate'); $('#js_comment{$aRow.comment_id}').find('.js_comment_moderate').remove(); $.ajaxCall('comment.moderateSpam', 'id={$aRow.comment_id}&amp;action=approve&amp;inacp={if isset($bIsCommentAdminPanel)}1{else}0{/if}'); return false;">{_p var='approve'}</a></li>
				<li class="js_comment_moderate"><a href="#" onclick="$('#js_comment{$aRow.comment_id}').find('.comment_header').removeClass('row_moderate'); $('#js_comment{$aRow.comment_id}').find('.js_comment_moderate').remove(); $.ajaxCall('comment.moderateSpam', 'id={$aRow.comment_id}&amp;action=deny&amp;inacp={if isset($bIsCommentAdminPanel)}1{else}0{/if}'); return false;">{_p var='deny'}</a></li>
			{/if}
			{if !isset($bIsCommentAdminPanel)}
	...
	<div id="js_comment_parent_view_{$aRow.comment_id}">
		<div style="text-align:right; margin-bottom:8px;">
			<a href="#" onclick="$(this).parent().html($.ajaxProcess()); $.ajaxCall('comment.getChildren', 'comment_id={$aRow.comment_id}&amp;type={$sType}'); return false;">{_p var='view_replies_total_to_this_comment' total=$aRow.child_total}</a>
		</div>
	</div>
	{/if}	
</div>