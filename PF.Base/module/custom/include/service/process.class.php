...
{	
	...
	{		
        ...
        foreach ($aVals['name'] as $iId => $aText) {
            ...
            $sVarName = Language_Service_Phrase_Process::instance()->prepare($aText['text']);
            break;
        }
        ...
        if ($this->database()->isField(($sCustomTable), Custom_Service_Custom::instance()->getAlias() . $sFieldName)) {
		...
		if ( $bAddField && 
			!$this->database()->isField(($sCustomValueTable), Custom_Service_Custom::instance()->getAlias() . $sFieldName))
		{
			$this->database()->addField(array(
					'table' => ($sCustomValueTable),
					'field' => Custom_Service_Custom::instance()->getAlias() . $sFieldName,
					'type' => $sValueTypeName
				)
			);		
		...
		if (!$this->database()->isField((Phpfox::getT('user_custom')), Custom_Service_Custom::instance()->getAlias() . $sFieldName))
		{			
			$this->database()->addField(array(
		...
		// Add the new phrase
		if (!Language_Service_Phrase_Phrase::instance()->isValid($sVarName))
		{
	...
	{		
        ...
        foreach ($aVals['name'] as $sKey => $aPhrases) {
            foreach ($aPhrases as $sLang => $aValue) {
                if (Language_Service_Phrase_Phrase::instance()->isValid($sKey, $sLang)) {
                    Language_Service_Phrase_Process::instance()->updateVarName($sLang, $sKey, $aValue['text']);
                }
            }
        }
        ...
        if (isset($aVals['current'])) {
            ...
            foreach ($aVals['current'] as $sKey => $aPhrases) {
                ...
                foreach ($aPhrases as $sLang => $aValue) {
                    ...
                    if (Language_Service_Phrase_Phrase::instance()
                            ->isValid($sKey . '_feed', $sLang) && isset($aValue['feed'])
                    ) {
                        Language_Service_Phrase_Process::instance()
                            ->updateVarName($sLang, $sKey . '_feed', $aValue['feed']);
                    }
                }
            }
        }
	...
	{		
		...
		foreach ($aFields as $aField)
		{
			foreach ($aVals as $iFieldId => $aVal)
			{
				...
				if ($aField['var_type'] == 'text' || $aField['var_type'] == 'textarea')
				{
					...
					if ((
							isset($aUserValues[Custom_Service_Custom::instance()->getAlias() . $aField['field_name']] )
							&& md5($aVal) == md5($aUserValues[Custom_Service_Custom::instance()->getAlias() . $aField['field_name']])
						))
					{		
						continue;
					}
		...
		{
		    Feed_Service_Process::instance()->delete('custom', $iItemId);
		    Feed_Service_Process::instance()->add('custom', $iItemId);
		}	
		...
		switch ($aField['var_type'])
		{
		 case 'radio':
			...
			case 'textarea':				
				...
				if (!$this->database()->isField(Phpfox::getT($sTable), Custom_Service_Custom::instance()->getAlias() . $aField['field_name']))
				{
					$this->database()->addField(array(
						'table' => Phpfox::getT($sTable),
						'field' => Custom_Service_Custom::instance()->getAlias() . $aField['field_name'],
						'type' => $sTypeName
							)
					);
				}
				...
				if (!$this->database()->isField(Phpfox::getT($sValueTable), Custom_Service_Custom::instance()->getAlias() . $aField['field_name']))
				{
					$this->database()->addField(array(
				...
				$aExisting = $this->database()->select('user_id as userCustom, ' . Custom_Service_Custom::instance()->getAlias() . $aField['field_name']  . ' as currentValue')
				...
				if (isset($aExisting['userCustom']) && !empty($aExisting['userCustom']))
				{
					$this->database()->update(Phpfox::getT($sTable), array(
                        Custom_Service_Custom::instance()->getAlias() . $aField['field_name'] => $sInsertValue
					), 'user_id = ' . (int)$iItemId);
				}
				else
				{
					$this->database()->insert(Phpfox::getT($sTable),array('user_id' => (int)$iItemId, Custom_Service_Custom::instance()->getAlias() . $aField['field_name'] => $sInsertValue));
				}
				...
				{
					$this->database()->insert(Phpfox::getT($sValueTable),array('user_id' => (int)$iItemId, Custom_Service_Custom::instance()->getAlias() . $aField['field_name'] => $sCleanValue));
				}					
	...
	public function delete($iId)
	{
		...
		if ($this->database()->isField(Phpfox::getT($sTable), Custom_Service_Custom::instance()->getAlias() . $aField['field_name']))
		{
			$this->database()->dropField(Phpfox::getT($sTable), Custom_Service_Custom::instance()->getAlias() . $aField['field_name']);
		}
		
		if ($this->database()->isField(Phpfox::getT($sValueTable), Custom_Service_Custom::instance()->getAlias() . $aField['field_name']))
		{
			$this->database()->dropField(Phpfox::getT($sValueTable), Custom_Service_Custom::instance()->getAlias() . $aField['field_name']);
		}
	...
	public function deleteOption($iId)
	{
		...
		{
            Language_Service_Phrase_Process::instance()->delete($aOption['phrase_var_name']);
		...
		$sFieldName = Custom_Service_Custom::instance()->getAlias() . $aOption['field_name'];
	...
	{	    
		...
		foreach ($aVals['option'] as $iKey => $aOptions)
		{
		    ...
		    foreach ($aOptions as $sLang => $aOption)
		    {
				...
				{
                    Language_Service_Phrase_Process::instance()->add([
                        'var_name' => 'cf_option_' . Language_Service_Phrase_Process::instance()->prepare($aOption['text']) . '_feed',
